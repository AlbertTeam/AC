...
function OrgShow(OrgNode_) {
    ...
    this.ShowType = null;
    var c_node_num= 0,      //存储横向节点数
        l_node_num = 0;     //存储纵向节点数
    ...
    this.Run = function() {
        ...
        for (var n = 1; n <= this.Depth; n++) { //设置横线条
            var tempNodes = this.DepthGroup[n].Nodes;
            // console.log(this.DepthGroup[n].Nodes)
            for (var m = 0; m < tempNodes.length; m++) {
                if (n != this.Depth) { //设置底线条
                    if (tempNodes[m].Nodes.length != 0) {
                        this.Status = OrgNode_.Status;  //显示查询状态
                        //根据设计稿设置线条居中(所以横线偏移)，取整防止小数
                         // var tempLineTop = tempNodes[m].Top + (tempNodes[m].Height / 2);  
                        var tempLineTop = parseInt(tempNodes[m].Top + (tempNodes[m].Height / 2)-50);   
                        var tempLineWidth = ((this.IntervalWidth - this.LineSize) / 2);
                        // var tempLineWidth = this.IntervalWidth - this.LineSize;
                        tempLineWidth += (this.DepthGroup[n].Width - tempNodes[m].Width);
                        var tempLineLeft = tempNodes[m].Left + tempNodes[m].Width;
                        var tempBottomLine = new CreateLine(3, tempLineTop, tempLineLeft, tempLineWidth, this.LineSize, this.LineColor, "LineBottom_" + tempNodes[m].Id,tempNodes[m].Status);
                ...
                if (n != 1) { //设置顶线条
                    ...
                    if (this.DepthGroup[tempNodes[m].Depth].NodeGroups[tempNodes[m].NodeGroupId].length == 1) { //如果只有一个节点
                        ...
                        tempNodes[m].parentNode.BottomLine.style.width = (tempLineWidth + tempBottomLineWidth) + "px";
                         var temptopLine = new CreateLine(1, tempLineTop, tempLineLeft, tempLineWidth, this.LineSize, this.LineColor, "LineTop_" + tempNodes[m].Id,tempNodes[m].Status);
                    } else {
                        var temptopLine = new CreateLine(1, tempLineTop, tempLineLeft, tempLineWidth, this.LineSize, this.LineColor, "LineTop_" + tempNodes[m].Id,tempNodes[m].Status);
                ...
                // 判断最后一个节点
                if(n == this.Depth || tempNodes[m].Nodes.length == 0){
                	// 固定最高尺寸
                	if(c_node_num <3 ){
						c_node_num +=1;
                	}
                    //判断横向节点数
                    l_node_num < tempNodes[m].Depth ? l_node_num = tempNodes[m].Depth : '';
                    console.log('设置tempNodes[m].Depth:',tempNodes[m].Depth);
                    console.log('设置l_node_num:',l_node_num);
                	var tempLineLeft = tempNodes[m].Left + 130;
                    var tempLineTop = tempNodes[m].Top + (tempNodes[m].Height / 2)-50;   
                	var lastLine = new CreateLine(4, tempLineTop, tempLineLeft, tempLineWidth, this.LineSize, this.LineColor, "LineBottom_" + tempNodes[m].Id,tempNodes[m].Status);
                	 tempNodes[m].TopLine = lastLine.Line;
                }
            }
        }
        ...
        // 通过节点高度，设置对应的高度
	    $('#box_test').css({
	        height: c_node_num*202
	    });
        // 设置拖动(条件：横向节点数超过4或纵向节点数超过3，才展示效果)
        if(c_node_num >= 3 || l_node_num > 4){
            $("#box_test").niceScroll( {
                touchbehavior: true,
                cursorcolor: "#FF00FF",
        ...
        for (var n = 2; n <= this.Depth; n++) { //设置竖线条
            ...
            for (var m = 0; m < tempNodeGroups_.length; m++) {
                if (tempNodeGroups_[m].length != 1) {
                    ...
                    var tempLineTop = tempNodeGroups_[m][0].Top + (tempNodeGroups_[m][0].Height / 2)-50;
    ...
    function GetBoxTemplet() { //取得节点模板
		...
		if (This.ShowType == 2){
  		...
  		var TempletStr = "<div class='OrgBox' id=\"{Id}\">" 
						+ " <img src='http://www.whmall.com:8080/SearchService.svc/rest/pic400x300/png/CCOC(C)=O' alt=''>" 
						+ "<p>CAS:120-46-7</p>" 
						+ "<p>MW:224.259</p>" 
						+ "<p>\{Description}\</p>"
						+ "<a href='#' class='historical_offer_btn J_historical_offer_btn'>历史报价</a>" 
						+ "</div>";
        return TempletStr;
    }
    //创建线条
    function CreateLine(type_, top_, left_, long_, size_, color_, id_ ,status_) {
        ...
        this.Status = (status_ == undefined) ? "精确路线" : status_;
        ...
        if (this.Type == 1) {
            ...
            // console.log(status_);
            // 设置线条html
	        var line_template="<div class='{precise}'>"
		        					+"<i class='arrows'>&lt;</i>"
					                +"<a href='#' class='btn'>\{Status}\</a>"
					                +"<div class='hint_message'>"
					                 +   "<div class='msg'>"
					                     +   "<p><i class='line_icon1'></i>共68种路线，已选择#1路线<a href='#'>更换</a></p>"
					                      +  "<p><i class='line_icon2'></i>共35种反应条件，已选择#1反应条件<a href='#'>查看</a><a href='#' class='J_show_syntheticnew_slide'>更换</a></p>"
					                    +"</div>"
					                +"</div>"
					            +"</div>";
	            //通过不同功能展示不同样式
	        this.Status=='智能分析'? line_template =line_template.replace('{precise}', 'capacity'):line_template =line_template.replace('{precise}', 'precise');       
	        line_template = line_template.replace('{Status}', this.Status);
	       $(this.Line).html(line_template);
        } else if (this.Type == 4) {
        	// 渲染尾部
            this.Line.style.borderTopWidth = this.Size;
            this.Line.style.borderTopStyle = "solid";
        var tail_template= "<div class='state'>"
				                +"<i class='plus_sign'>+</i>"
				                 +"<div class='hint_message'>"
				                    +"<div class='msg'>"
					                    +"<p>共68种<a href='#' class='btn1'>精确路线</a></p>"
					                    +"<p>共35种<a href='#' class='btn2'>智能分析</a></p>"
				                    +"</div>"
				                +"</div>"
				            +"</div>";
       		$(this.Line).html(tail_template);
